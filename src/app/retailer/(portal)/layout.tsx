...
export default async function RetailerPortalLayout({ children }: { children: React.ReactNode }) {
  await requirePortalSession("retailer", "/retailer/login");
    ...
    <div className="min-h-screen bg-background text-foreground">
      <aside className="fixed inset-y-0 left-0 z-[60] hidden md:flex flex-col bg-secondary-container h-full w-72 shadow-2xl">
        ...
        <div className="p-4 border-t border-white/10">
          <form action={signOutAndRedirect.bind(null, "/retailer/login")}>
            <button
              type="submit"
              className="w-full flex items-center justify-center gap-2 p-3 text-on-secondary-container/70 font-medium hover:bg-white/10 rounded-lg transition-colors text-sm"
            >
              Logout
            </button>
          </form>
