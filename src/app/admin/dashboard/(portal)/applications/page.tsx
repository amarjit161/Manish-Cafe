  ...
  return (
    <div className="space-y-4">
      ...
      {applications.length === 0 ? (
        ...
        <>
          {/* Mobile: cards -- the 9-column table doesn't fit a phone width,
              and squeezing it just makes every cell truncated and useless. */}
          <div className="md:hidden space-y-3">
            {applications.map((app) => (
              <Link
                key={app.id}
                href={`/admin/dashboard/applications/${app.id}`}
                className="block rounded-xl border border-outline-variant bg-surface-container-lowest p-4 space-y-2 hover:border-primary/40 transition-colors"
              >
                <div className="flex items-start justify-between gap-2">
                  <span className="font-semibold text-primary">{app.application_number ?? "—"}</span>
                ...
                <div className="flex items-center justify-between text-label-sm text-on-surface-variant pt-1 border-t border-outline-variant">
                  <span>
                    {app.documentCounts.approved}/{app.documentCounts.total} docs · ₹{app.customer_price_snapshot}
                  </span>
          ...
          <div className="hidden md:block overflow-x-auto rounded-xl border border-outline-variant">
            <table className="w-full text-left text-body-md">
              ...
              <tbody>
                {applications.map((app) => (
                  <tr key={app.id} className="border-t border-outline-variant hover:bg-surface-container-low/50 transition-colors">
